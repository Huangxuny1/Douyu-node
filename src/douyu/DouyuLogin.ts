...
const headerReferer = 'https://passport.douyu.com/index/login?passport_reg_callback=PASSPORT_REG_SUCCESS_CALLBACK&passport_login_callback=PASSPORT_LOGIN_SUCCESS_CALLBACK&passport_close_callback=PASSPORT_CLOSE_CALLBACK&passport_dp_callback=PASSPORT_DP_CALLBACK&type=login&client_id=1&state=https%3A%2F%2Fwww.douyu.com%2F&source=click_topnavi_login';
...
export default class DouyuLogin {
    ...
    public doLogin = async () => {
        ...
        const fileName: string = 'cookie.txt'
    ...
    private _login = async () => {
        return new Promise((resolve, reject) => {
            requests.post(generatoeCodeUrl)
    ...
    private _checkScan = async () => {
        ...
        return new Promise((resolve, reject) => {
            const checkInterval = setInterval(() => {
                requests.get(util.format(util.format("https://passport.douyu.com/lapi/passport/qrcode/check?time=%s&code=%s", new Date().getTime(), data.data.code)))
                    .set('Referer', headerReferer)
                    .set('User-Agent', headerUserAgent)
    ...
    private _getCookie = async () => {
        ...
        return new Promise((resolve, reject) => {
            requests.head('https:' + data.data.url + "&callback=appClient_json_callback&_=" + new Date().getTime())
                .set("Accept", "application/json, text/javascript, */*; q=0.01")
